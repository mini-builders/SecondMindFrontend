...
async def clear_completed_tasks_route(
    ...
    user_id = str(current_user["_id"])
    deleted = await delete_completed_tasks(user_id)
    logger.info("Cleared completed tasks | user_id=%s | count=%d", user_id, deleted)
...
async def update_task_route(
    ...
    if request.priority is not None:
        if request.priority not in ["high", "medium", "low"]:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Priority must be 'high', 'medium', or 'low'.")
